...
module.exports.run = async function({ api, event, args, client, __GLOBAL }) {
	...
	if (!text) return api.sendMessage("PORNHUB-এর জন্য অনুগ্রহ করে একটি মন্তব্য লিখুন \n- { সিয়াম রহমান }", threadID, messageID);
	let getAvatar = (await axios.get(linkAvatar, { responseType: 'arraybuffer' })).data;
	let getPorn = (await axios.get(`https://raw.githubusercontent.com/SIAMRAHMAN000/__IMAGES__/main/phub.png`, { responseType: 'arraybuffer' })).data;
